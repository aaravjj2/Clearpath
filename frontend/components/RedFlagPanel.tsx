...
export default function RedFlagPanel({ clauses, loading }: RedFlagPanelProps) {
  ...
  if (clauses.length === 0) {
    return (
      <div
        data-testid="no-red-flags"
        className="bg-slate-900 rounded-xl border border-slate-800 p-8 text-center"
      >
        <AlertTriangle className="w-8 h-8 text-green-400 mx-auto mb-3" aria-hidden="true" />
        <p className="font-medium text-slate-200">No red flags detected</p>
        <p className="text-sm text-slate-500 mt-1">
          This does not replace legal advice, but there are no obvious high-risk clauses.
        </p>
      </div>
    );
  }
  ...
  const sorted = [...clauses].sort(
    (a, b) => (b.red_flag?.severity ?? 0) - (a.red_flag?.severity ?? 0)
  );

  return (
    <div
      data-testid="red-flag-panel"
      aria-label={`${sorted.length} red flag${sorted.length !== 1 ? "s" : ""} detected`}
      className="space-y-3"
    >
      {sorted.map((clause) => (
