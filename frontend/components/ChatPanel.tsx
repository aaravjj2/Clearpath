...
export default function ChatPanel({ documentId, disabled }: { documentId: string; disabled: boolean }) {
  const [messages, setMessages] = useLocalStorage<ChatMessage[]>(`chat-${documentId}`, []);
  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-800 flex flex-col h-[500px]">
      <div className="flex items-center justify-between px-4 pt-3 pb-1 border-b border-slate-800">
        <span className="text-xs text-slate-500 uppercase tracking-wide">Chat</span>
        {messages.length > 0 && (
          <button
            onClick={() => setMessages([])}
